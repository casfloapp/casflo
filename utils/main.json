...
// Bungkus semua kode dalam pengecekan ini
    ...
    function initializePageEventListeners() {
        ...
        function renderCalendar() {
            ...
            for (let i = 1; i <= daysInMonth; i++) {
                ...
                let dailyTransactions = window.appState.transactions.filter(t => new Date(t.date).toDateString() === date.toDateString());
        ...
        function renderTransactionList(filteredTransactions) {
            ...
            listHeader.innerHTML = `
                <div class="date-info"><i class="fas fa-chevron-down fa-xs"></i><span>Hari Ini</span></div>
                <div class="summary">Pengeluaran: Rp${totalExpenseToday.toLocaleString('id-ID')}</div>
            `;
            ...
            filteredTransactions.forEach(t => {
                ...
                itemEl.innerHTML = `<div class="icon" style="background-color: ${t.color};"><span>${t.icon}</span></div><div class="details"><div class="category">${t.category}</div><div class="notes">${t.notes}</div></div><div class="info"><div class="amount" style="color: ${t.type === 'expense' ? 'var(--expense-red)' : 'var(--income-green)'}">${t.type === 'expense' ? '-' : ''}Rp${t.amount.toLocaleString('id-ID')}</div><div class="account">${t.account}</div></div>`;
